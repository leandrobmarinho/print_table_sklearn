    ...
    for i in range(0, int(len(df.index) / 2)):
        ...
        for metric in metrics:
            ...
            if metric is 'score_time' or metric is 'fit_time':
                output_prec = output.replace('PRECISION', '1G')
            else:
...
if __name__ == '__main__':
    ...
    metrics = ['test_accuracy','fit_time', 'score_time']
    # ,'fit_time', 'score_time', 'test_f1_macro', 'test_recall_macro', 'test_precision_macro'

    type = 'mean_std_metrics_pm'  # mean_std_metrics mean_metrics test_acc mean_std_metrics_pm

    num_test = 120
    for file in glob.glob('/Users/leandrobm/Desktop/TRAFO_DETECTION/Metrics/out-Quatro-classes-TrafoDataset/*.csv'):
